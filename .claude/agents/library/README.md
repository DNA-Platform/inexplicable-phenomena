...
The library uses a two-layer model. Agent libraries live under `..team/` (synced from dna-library, extended locally). Shared books (coding-policy, .librarianship) live at the root.

```
.claude/team/library/
  README.md                  — you are here
  .librarianship/            — meta: how to run a library
    .cover.md                — book cover
    00-the-library.md        — chapters...
  coding-policy/             — shared: team coding standards
    .cover.md
    01-naming.md
    ...
  ..team/                    — agent libraries (from dna-library)
    {agent-name}/            — one agent's library
      README.md              — agent's library intro
      .perspective/          — visual observations (optional)
      {book-slug}/           — a book (directory)
        .cover.md            — book frontmatter + introduction
        01-{chapter-slug}.md — chapters (ordered by numeric prefix)
        02-{chapter-slug}.md
        ...
```

Each agent **authors** the books in their own library. Authorship is recorded in the book's `.cover.md` frontmatter as a link to the agent's file at `.claude/team/agents/{agent}.md`.

The `..team/` prefix keeps agent libraries sorted before alphabetical directories in file listings and signals that this layer is managed by dna-library. Local extensions (new chapters, updated covers) are written here and synced back.

## Book frontmatter

The book's `.cover.md` carries the frontmatter:
