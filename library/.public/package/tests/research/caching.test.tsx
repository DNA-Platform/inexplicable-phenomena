...
// Characterization probes — how a memoized derivation behaves when its reactive
// source changes, and whether a render-time-writing getter is safe. Handlers use
// inline arrows so binding is never the variable; PROBE 6 isolates binding itself.
...
describe('research: caching a derivation across a reactive source change', () => {
    it('PROBE 4 — a naive `??=` memo goes STALE when the source changes', async () => {
        ...
        expect(container.querySelector('.out')!.textContent).toBe('a,b');   // stale
    });
    ...
    it('PROBE 5 — a cache KEYED to the source recomputes on change, not per render', async () => {
        ...
        class $S extends $Chemical {
            ...
            _cache?: string[];
            get words(): string[] {
                if (this._key !== this.$text) { computes++; this._key = this.$text; this._cache = this.$text.split(' '); }
                return this._cache!;
            }
            view() {
                return (
        ...
        expect(computes).toBe(afterMount);                     // no recompute on a plain re-render

        await act(async () => { fireEvent.click(container.querySelector('.chg')!); });
        expect(container.querySelector('.out')!.textContent).toBe('x,y,z');
        expect(computes).toBe(afterMount + 1);                 // recompute exactly once, on change
    });
    ...
    it('PROBE 6 — does a getter that WRITES during render break `onClick={this.method}` binding?', async () => {
        class $S extends $Chemical {
            $text = 'hi';
            _c?: string;
            get derived(): string { return (this._c = this.$text.toUpperCase()); }  // writes during render
            act() { this.$text = 'bye'; }
            view() {
                return (
                    <div>
    ...
    it('PROBE 7 — a keyed cache that MINTS parts re-mints on change, no runaway', async () => {
        class $Word extends $Chemical {
            $label = '';
            view() { return <span className="w">{this.$label}</span>; }
        }
