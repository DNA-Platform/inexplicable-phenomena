        ...
        return (
            <>
                <Section>
                    ...
                    {'\n\nAnd the showing, once it was looked at closely, turned out to need less than it had been given. The book that draws is the one already to hand: a book knows its chapters and knows the order they stand in, so drawing them in that order is not something anybody has to write. What the application adds is a way to turn a path into a card, a way to fetch one book, and a surface — and a surface is the easiest thing in the whole arrangement to change, which is the argument for choosing a plain one first and a good one later.'}
                    {'\n\nTwo smaller things fall out of the same shape. A subject can be a page of its own, because consulting a catalogue opens nothing: a subject costs its own chapters and the cards of what it holds, and never the books themselves. And where a reader stands can be kept by the page, because a library served as files has nowhere else to keep it — which is little enough to describe in a sentence and enough to bring somebody back to the chapter they left.'}
                </Section>
