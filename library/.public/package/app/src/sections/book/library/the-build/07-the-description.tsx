        ...
        return (
            <>
                <Section>
                    ...
                    <Paragraph>{'One stage reads the folders and every stage after it reads what that stage produced. So the description is the single agreement the whole machine turns on, and it is worth being exact about, because two people cannot build either side of a thing that has never been written down.'}</Paragraph>
                    <Paragraph>{'It is a flat list, not a tree. Every folder appears once, carrying its own path, how many dots it wears, whether that makes it a subject or a book, which of the folders inside it speaks for it, what it holds, and what files it contains with the role of each. The hierarchy is not stored because the paths already have it, and a flat list can be written to a file, compared against yesterday, and read by something that was not compiled with it.'}</Paragraph>
                    <Paragraph>{'Paths are the identity. They are relative to the top of the library and they use one kind of slash, because the same string that names a folder is the string a route will arrive holding, and having two spellings of one thing is how a system begins to disagree with itself.'}</Paragraph>
                    <Description
                        paths={corpus}
                    ...
                    />
                    <Paragraph>{'Order is resolved here and never again. Reading applies the arrangement a person made and puts anything unarranged after it, so no later stage has to know that ordering was ever a question. A stage that re-derived order would be a second opinion about it, and two opinions are one more than a library can afford.'}</Paragraph>
                    <Paragraph>{'What the description does not contain is the writing. It says a file is a chapter; it does not say what the chapter says. Reading looks at names and arrangement only, which is what makes it fast, and what makes it possible to check an entire library before a single line of anybody’s prose has been compiled.'}</Paragraph>
                    <Paragraph>{'And complaints travel with it rather than stopping it. A folder marked a subject that holds nothing, a folder where no single entry speaks for the rest, a book with no cover — each is collected and carried, so one pass tells an author everything that is wrong. A build that reports one fault at a time is a build somebody runs many times, and each run is a chance to give up.'}</Paragraph>
                    <Description
                        paths={broken}
                    ...
                    />
                    <Paragraph>{'That is the whole of it, and its smallness is the point. Everything downstream — the copying, the assembling, the judging, the cataloguing — reads this and never the filesystem, which is why those four can be built by four people who never speak.'}</Paragraph>
                    <Listed
                ...
                <Summary>
                    <Title>Summary</Title>
                    <Paragraph>{'A flat list of folders, keyed by path, with order resolved and complaints carried — the one agreement the whole machine turns on.'}</Paragraph>
                </Summary>
            </>
        );
    }
