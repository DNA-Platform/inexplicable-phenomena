...
describe('a piece of writing says what kind of writing it is', () => {
    ...
    it('and writing with no kind written into it answers nothing', () => {
        expect(built<$Writing>(<Writing>a</Writing>).kind).toBeUndefined();
    });
...
describe('a piece of writing holds nothing above its own level', () => {
...
describe('an annotation weighs in; a type also resolves', () => {
    ...
    it('AND A TYPE CARRIES NO KIND OF ITS OWN', () => {
        expect(built<$Type>(<TypeOfBook />).kind).toBeUndefined();
    });
...
describe('a mention stands for another piece of writing', () => {
    ...
    it('AND A MENTION THAT STANDS FOR WHAT IT HOLDS NEEDS NO PATH — every word the parse makes has one', () => {
        const held = built<$Composition>(<Paragraph>One thing to say.</Paragraph>);
        const words = held.parts().flatMap(s => (s as $Composition).parts());
...
describe('a format is worn by writing', () => {
    it('and it makes its own theme, so one worn by nothing is still drawn in something', () => {
        const bare = built<$Format>(<Format />);
        expect(bare.theme).toBeInstanceOf($Theme);
    });
});

describe('a piece of writing is specified as it is bonded', () => {
    class $Watched extends $Writing { }
    const Watched = $($Watched);

    it('and a rule its own type carries has already run', () => {
        const ran: $Writing[] = [];
        class $TypeOfWatched extends $Type {
            override name = 'Watched';
            override specifically(writing: $Writing): void { ran.push(writing); }
        }
        const TypeOfWatched = $($TypeOfWatched);
        const watched = built<$Writing>(<Watched><TypeOfWatched />a</Watched>);
        expect(ran).toContain(watched);
    });

    it('AND A RULE THAT REFUSES THE WRITING REFUSES IT WHERE IT IS DRAWN', () => {
        class $TypeOfRefused extends $Type {
            override name = 'Refused';
            override specifically(): void { throw new Error('a watched writing is refused, and this one was'); }
        }
        const TypeOfRefused = $($TypeOfRefused);
        const refused = built<$Writing>(<Watched><TypeOfRefused />a</Watched>);
        const Drawn = $(refused);
        expect(render(<Drawn />).container.textContent).toContain('this one was');
    });
});

describe('a composition generates the level it needs from what it holds', () => {
    ...
    it('and the heading it generates reads as the first sentence, elided', () => {
        const heading = section().parts()[0];
        expect(reflection.is(heading, $TypeOfHeading)).toBe(true);
        expect(html.text(heading._block)).toContain('One thing to say.');
        expect(html.text(heading._block)).not.toContain('Then another thing.');
    });

    it('AND A SECTION WRITTEN WITH ITS OWN HEADING KEEPS IT, ungenerated', () => {
        const held = built<$Composition>(
            <Section><Heading>Written by hand</Heading><Paragraph>One thing to say.</Paragraph></Section>);
        expect(html.text(held.parts()[0]._block)).toBe('Written by hand');
    });

    it('AND A SECTION WITH NOTHING TO READ IS STILL REFUSED — generation does not excuse an empty one', () => {
        expect(() => built<$Composition>(<Section />).specify()).toThrow();
    });
    ...
    it('AND A CHAPTER TAKES A LETTER — the theorem: a composition takes anything at or below its own level', () => {
        expect(() => built<$Composition>(<Written>a</Written>).specify()).not.toThrow();
    });
    ...
    it('AND YOU STILL END UP WITH A TITLE — the section a chapter reads runs its own type, so it has its heading', () => {
        ...
        expect(reflection.is(section.parts()[0], $TypeOfHeading)).toBe(true);
    ...
    it('and asking twice answers the same parts, because a reading is not a rewriting', () => {
        const held = section();
