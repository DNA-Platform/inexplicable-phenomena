...
import { $Writing, Writing, $Type, TypeOfParagraph, $TypeOfParagraph, TypeOfSection, Specification, specify } from '@dna-platform/public';
...
describe('a piece of writing carries one of the seven, and as many other kinds as it likes', () => {
    it('a paragraph that is also a list stands at the paragraph level', () => {
        ...
        expect(list.kind).toBeInstanceOf($TypeOfParagraph);
    });
    ...
    it('and a kind can be swapped by writing a different one in', () => {
        ...
        expect(paragraph.kind.name).toBe('Paragraph');
        expect(section.kind.name).toBe('Section');
    });
});
