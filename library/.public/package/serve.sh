#!/bin/sh
# THE DEMO SERVERS — each serves the SITE its .binding built, the way Pages will. Vite reads the package through dist, and its module graph caches that transform,
...
(cd "$P/.latex/.binding" && npx vite preview --port 5310 --strictPort >/dev/null 2>&1) &
(cd "$P/.wiki/.binding" && npx vite preview --port 5311 --strictPort >/dev/null 2>&1) &
sleep 12
