...
const seams: Handoff[] = [
    ...
    },
    {
        ...
        agreement: 'A verdict, and the program it stood over. Checking invents no rules: the bond constructors already fail six malformations, so it is construction, watched.',
        ...
        return (
            <>
                <Section>
                    ...
                    <Paragraph>{'The phases run in one order and that is not the order they have to be built in. Data flows forwards; work does not. What decides whether two people can build at once is not the flow but the agreement between them — and an agreement is only real when the receiving side can build against something today, without waiting for the sending side to exist.'}</Paragraph>
                    ...
                    <Paragraph>{'The reason every row can name one is that a hand-made version of this library already exists. Its books were composed by a person, its catalogue typed out, its links wired by hand — and that is exactly the shape each later phase consumes. What was built to find out whether the idea worked turns out to be the input the second half of the machine needs.'}</Paragraph>
                    <Paragraph>{'So the seams are what get agreed first, and after that the work divides on its own. Somebody writes the reader while somebody else writes the thing that reads its output, and neither has to see the other. When both are done the hand-made version is deleted and the real one takes its place; if that swap is difficult, an agreement was wrong, and the difficulty is the report.'}</Paragraph>
                    ...
                    <Paragraph>{'There are two kinds of parallel here and they are worth telling apart, because an earlier arrangement confused them and produced one team in four costumes. Phases are sequential: nothing resolves before something has been read. But the ARTIFACTS a phase produces are independent of each other, and emitting produces three — the modules, the catalogue, the entry. Two people can build two artifacts of one phase without meeting, and neither is waiting on the other. That is where the room to divide actually is, and it survives a change of mind about layout, which the old arrangement did not.'}</Paragraph>
                    <Paragraph>{'One team goes first not because the others depend on it but because it pays to. Every other team begins against something written by hand, and the moment the first has landed, five hand-made inputs become one real artifact. Nothing was blocked by waiting; a great deal of pretending was ended by not.'}</Paragraph>
                    <Paragraph>{'Four of these have now been built, and the arrangement above turns out to have been optimistic about one thing and right about everything else. Reading, resolving, emitting and showing stand; the catalogue is still typed by a person and checking was built by whoever needed it rather than by whoever was dispatched to it. What was optimistic is the number of hands: the whole of resolving and emitting came to roughly two hundred lines, which is less work than the coordination it was cut into. A plan that divides work has to be checked against the size of the work, and this one was not until the work was measured.'}</Paragraph>
                    <Paragraph>{'The last cannot be brought forward, and that is its definition rather than its misfortune. Its work is to take out each hand-made input and put the real one in its place, which cannot happen until the real ones exist. It should be a small job, and if it is a large one an agreement further up was wrong — the size of it is the report.'}</Paragraph>
                    <Paragraph>{'One of the six is not in the seam figure, because nothing is handed onward from it. Showing takes a card and makes a page, and it is the only phase a person actually meets. Which argues for an order that looks backwards. Build the showing first, against the hand-made catalogue, and there is a working library on a screen before a single line of the machine is written. Build the machine first and there are finished phases and nothing to look at. The second is not slower, but it is unreviewable, and a thing nobody can look at is a thing nobody can correct.'}</Paragraph>
                    <Paragraph>{'Four agreements needed a mechanism before anyone could be sent to them, and each is now decided rather than deferred. Order lives in a manifest kept beside the library and read by the same tool that writes it, so an arrangement someone made by hand survives into a build nobody ran by hand. A cover that names no author has one supplied when the library is resolved — never written back into what the author wrote. Checking runs with no browser at all, which was measured rather than assumed: of sixty-two promises about books, fifty-eight hold with nothing but a language runtime, and the four that fail are the four that draw. And where emitted code lands is a policy rather than an agreement, which is why nobody is dispatched to it.'}</Paragraph>
                    <Paragraph>{'The last of them is about what a card can do when its book is elsewhere. A card carries a path and hands back no book, because it genuinely has none — and when the reader follows it, the page loads that one book and the card is handed it then. This is what following a card has always meant: a catalogue does not give you the volume, it tells you where the volume is, and once you have fetched it the card is standing beside it.'}</Paragraph>
                </Section>
                <Summary>
                    <Title>Summary</Title>
                    <Paragraph>{'Agreements divide the work, not order — and inside a phase, its artifacts divide it again.'}</Paragraph>
                </Summary>
            </>
        );
    }
