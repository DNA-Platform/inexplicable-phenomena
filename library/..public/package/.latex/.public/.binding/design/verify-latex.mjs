...
const reading = async (name) => {
    const seen = await page.evaluate(() => ({
        ...
        face: getComputedStyle(document.querySelector('main') ?? document.body).fontFamily,
    }));
    ...
    console.log(`${name}: ${seen.chars} chars · ${seen.rows.length} rows landing ${seen.rows.length - rowsHidden.length} · ${seen.citations.length} citations landing ${seen.citations.length - citationsHidden.length} · ${seen.entries} entries · ${seen.katex} KaTeX errors · ${seen.panels} panels · face ${seen.face.split(',')[0]}`);
    return seen;
};

const latex = await reading('latex');
...
if (markdown) {
    ...
    const other = await reading('markdown');
    // A SWITCH THAT SWITCHES NOTHING IS A DEAD CLIENT. Two readings of one document share every
    // count above, so the counts cannot see it; the face can. Added 2026-09-14 after a prerendered
    // page with no live client passed this gate on both readings.
    expect(latex.face !== other.face, `the switch changed nothing — main wears ${latex.face.split(',')[0]} under both readings`);
